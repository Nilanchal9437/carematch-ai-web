    ...
    fields: [
      {
        ...
        type: 'multiselect',
        options: ['Memory', 'Assisted Living', 'Respite', 'Nursing', 'Not sure'],
      },
      {
        name: 'dailyActivities',
        label: 'What daily activities do they need help with?',
        type: 'multiselect',
        options: ['Bathing', 'Dressing', 'Walking', 'Toileting', 'Medication', 'Eating', 'None'],
      },
      {
        name: 'conditions',
        label: 'Do they have any of the following conditions?',
        type: 'multiselect',
        options: ['Alzheimer\'s/Dementia', 'Stroke', 'Diabetes', 'Parkinson\'s', 'None'],
      },
      {
        name: 'mobilityAids',
...
export default function IntakeForm() {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      alert('Thank you — our team will reach out with 2–3 personalized matches within 24 hours.')
    } catch (error) {
        ...
        >
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            <div className="bg-white shadow rounded-lg p-6">
              <h3 className="text-xl font-semibold mb-6">{formSteps[currentStep].title}</h3>
